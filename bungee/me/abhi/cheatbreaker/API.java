...
public class API extends Plugin implements Listener {
    ...
    private void load() throws IOException {
        ...
        if (!f.exists()) {
            ...
            cfg.set("server-restart", "Everyone has been kicked for re-authentication.");
            cfg.set("recommendation-message", "&fUsing &c&lCheat&f&lBreaker &fis highly recommended. You may download the client at &chttps://cheatbreaker.com&f.");
            cfg.set("kick-message", "&cPlease use CheatBreaker to join the server! Download at cheatbreaker.com!");
            cfg.set("welcome-message", "&aThank you for using CheatBreaker!");
            cfg.set("no-permission", "&cNo permission.");
            cfg.set("could-not-find-player", "&cCould not find player.");
            cfg.set("player-is-on-cheatbreaker", "&a%player% is using CheatBreaker!");
            cfg.set("player-is-not-on-cheatbreaker", "&c%player% is not using CheatBreaker!");
